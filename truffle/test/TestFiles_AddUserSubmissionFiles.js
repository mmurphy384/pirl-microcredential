	...
	it("should create a contract, add 3 agencies, 6, files, retrieve files by agency, retrieve both.", function() {
		return Files.deployed().then(function(instance) {
			...
			_instance = Files.at(instance.address);
		 	...
		 	return _instance.getFileById.call(1);
		}).then(function (results) {
		 	console.log('######### Log: Check User 1 file 1 out.  Should be user1 File 1 not submitted');
			 console.log('######### Log: FileName = ' + toAscii(results[0]) + ' and UserSubmission=' + results[5].toNumber());
			 return _instance.updateFileById(
				 	1,
					files[0].name, 
					files[0].url, 
					files[0].pirlFileHash,
					true,
					files[0].userId,
					files[0].userSubmissionId, 
					files[0].agencyId,{from:accounts[users[0].account],gas: 19612388});
		}).then(function (result) {
			console.log('######### Log: 6 files created.  GasUsed = ' + result.receipt.gasUsed);
			return _instance.getFileById.call(1);
		}).then(function (results) {
			console.log('######### Log: Check User 1 file 1 out.  UserSubmission should be set');
			console.log('######### Log: FileName =  ' + toAscii(results[0]) + ' and UserSubmission=' + results[5].toNumber());
			assert.equal(results[5].toNumber(),1,'The user submission Id has been successfully updated');
		});
	});
